...
func (orm *ORM) Clone() Entity {
	...
	for i, field := range orm.tableSchema.fields.fields {
		...
		if field.IsExported() {
			newEntity.getORM().elem.Field(i).Set(orm.elem.Field(i))
		} else {
			for k := 0; k < orm.elem.Field(i).Type().NumField(); k++ {
				newEntity.getORM().elem.Field(i).Field(k).Set(orm.elem.Field(i).Field(k))
			}
		}
	}
	return newEntity
